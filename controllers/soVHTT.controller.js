...
async function getUniqueAdInfo(item_list) {
  ...
  for (let i = 0; i < item_list.length; i++) {
    if (
      unique_ad_types.includes(item_list[i].properties.type_advertise) == false
    ) {
      unique_ad_types.push(item_list[i].properties.type_advertise);
    }

    if (
      unique_land_types.includes(item_list[i].properties.place_type) == false
    ) {
...
const _change_report_status = async (req, res) => {
  ...
  try {
    await db
      .getDb()
      .collection("reports")
      .findOneAndUpdate({ _id: ObjectId(id) }, { $set: { state: state } });
